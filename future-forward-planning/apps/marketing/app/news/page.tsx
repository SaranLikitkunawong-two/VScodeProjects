...
import { ARTICLES, type ArticleSummary, type CoverKind } from "@/lib/articles";

const COVER_COLORS: Record<CoverKind, [string, string]> = {
  docs: ["#E5E8F4", "#3F51B5"],
  chart: ["#D5EAE7", "#00897B"],
  swap: ["#E5E8F4", "#3F51B5"],
  doc: ["#F1F3FA", "#3F51B5"],
...
function ArticleCover({ kind }: { kind: CoverKind }) {
        ...
        {kind === "chart" && (
          <g>
            ...
            {[40, 80, 120, 160, 200, 240, 280].map((x, i) => (
              <circle key={i} cx={x} cy={[120, 108, 92, 76, 64, 50, 42][i]} r="3.5" fill="#fff" stroke={fg} strokeWidth="2" />
            ))}
          </g>
        )}
        {kind === "swap" && (
        ...
        {kind === "doc" && (
          <g>
            ...
            {[66, 80, 94, 108, 122, 136].map((y, i) => (
              <rect key={i} x="116" y={y} width={88 - i * 4} height="4" rx="2" fill={fg} opacity="0.3" />
            ))}
...
function ArticleCard({ a, featured }: { a: ArticleSummary; featured: boolean }) {
  return (
    <article className={`article ${featured ? "featured-article" : ""}`}>
      ...
      <div className="article-body">
        ...
        <h3>{a.title}</h3>
        <p>{a.excerpt}</p>
        <Link href={`/news/${a.slug}`} className="read-more">
          Read more <ArrowIcon />
        </Link>
      </div>
    </article>
    ...
    <main id="main">
      <section className="news-hero">
        <div className="container">
          ...
          <p className="lede">
            Plain-English summaries of the pricing changes, compliance shifts, and participant guidance we think are
            worth your time.
          </p>
        </div>
      </section>
