...
const SEGMENTS = [
  { id: "total", label: "Total plan", color: "#3F51B5", share: 1.0 },
  {
    id: "core",
    label: "Core supports",
    color: "#00897B",
...
export default function BudgetCalculator() {
  const [planTotal, setPlanTotal] = useState("60000");
  const [months, setMonths] = useState("12");
  const [elapsed, setElapsed] = useState("5");
  const [active, setActive] = useState<string>("total");

  const [coreMonthly, setCoreMonthly] = useState("3800");
  const [capacityMonthly, setCapacityMonthly] = useState("420");
  const [capitalCommitted, setCapitalCommitted] = useState("2400");
  const [capitalPlanned, setCapitalPlanned] = useState("1500");

  const planTotalN = Number(planTotal) || 0;
  const m = Math.max(1, Number(months) || 1);
  const elapsedM = Math.min(m, Math.max(0, Number(elapsed) || 0));

  const coreCap = planTotalN * 0.8;
  const capacityCap = planTotalN * 0.1;
  const capitalCap = planTotalN * 0.1;
    ...
    <div className="calc-wrap" aria-label="Plan budget calculator">
      <div className="calc-header">
        ...
        <div className="calc-plan-meta">
          <label htmlFor="calc-plan-total" className="calc-mini-label">
            Annual plan
          </label>
      ...
      <div className="seg-tabs" role="tablist" aria-label="Budget segments">
        {SEGMENTS.map((s) => (
          ...
          >
            ...
            {s.id !== "total" && <span className="seg-share">{Math.round(s.share * 100)}%</span>}
          </button>
        ))}
      </div>
      ...
      {active === "total" && (
        <TotalView
          ...
          coreCap={coreCap}
          coreSpent={coreSpent}
          coreMonthly={Number(coreMonthly) || 0}
          capacityCap={capacityCap}
          capacitySpent={capacitySpent}
          capacityMonthly={Number(capacityMonthly) || 0}
          capitalCap={capitalCap}
          capitalCommitted={capitalCommittedN}
          capitalPlanned={capitalPlannedN}
      ...
      {active === "core" && (
        <RunRateView
          ...
          elapsed={elapsedM}
          monthly={coreMonthly}
          setMonthly={setCoreMonthly}
          setElapsed={setElapsed}
          spent={coreSpent}
          projected={coreProjected}
          unitLabel="month"
      ...
      {active === "capacity" && (
        <RunRateView
          ...
          setMonthly={setCapacityMonthly}
          setElapsed={setElapsed}
          spent={capacitySpent}
          projected={capacityProjected}
          unitLabel="month"
      ...
      {active === "capital" && (
        <CapitalView
          ...
          setCommitted={setCapitalCommitted}
          planned={capitalPlanned}
          setPlanned={setCapitalPlanned}
        />
      )}
    </div>
  );
...
function TotalView({
  ...
  const order: Record<Tone, number> = { neutral: 0, ok: 1, warn: 2, alert: 3 };
      ...
      <div className="calc-inputs">
        ...
        <div className="field" style={{ marginBottom: 0 }}>
          ...
          <div className="status-row">
            <span>Month 0</span>
            <span>
              <strong style={{ color: "var(--ink)" }}>{elapsed}</strong> of {months}
            </span>
  ...
  return (
    <button type="button" className="seg-summary" onClick={onClick}>
      <div className="seg-summary-hd">
        ...
        <span className={`${TONE_CHIP[tone]} seg-summary-chip`}>{Math.round(pct)}% used</span>
      </div>
      ...
      <div className="seg-summary-meta">
        <span>
          {formatAUD(used)} of {formatAUD(cap)}
        </span>
        ...
        <div style={{ marginTop: 18 }}>
          ...
          <div className={`progress ${TONE_PROGRESS[status.tone]}`}>
            <span
              style={{
                width: remainingPct + "%",
                background: status.tone === "ok" || status.tone === "neutral" ? segment.color : undefined,
              }}
            ></span>
          </div>
        </div>
    ...
    <div className="calc-body">
      <div className="calc-inputs">
        <div className="seg-intro" style={{ borderLeftColor: segment.color }}>
          ...
          <p>
            Capital supports are one-off purchases — assistive tech, equipment, modifications. Track them as discrete
            amounts against your annual pool, not a monthly run-rate.
          </p>
        </div>
        ...
        <div className="field">
          ...
          <div className="hint" style={{ marginTop: 6 }}>
            Equipment already paid for or under quote.
          </div>
        </div>
        ...
        <div className="field" style={{ marginBottom: 0 }}>
          ...
          <div className="hint" style={{ marginTop: 6 }}>
            e.g. assistive tech you intend to order this plan year.
          </div>
        </div>
      </div>
      ...
      <div className="calc-outputs">
        ...
        <div style={{ marginTop: 18 }}>
          ...
          <div className="stack-bar" aria-hidden>
            ...
            <span
              className="stack-planned"
              style={{ width: plannedPct + "%", background: segment.color, opacity: 0.45 }}
            ></span>
          </div>
          <div className="stack-legend">
            <span>
              <span className="legend-sq" style={{ background: segment.color }}></span>Committed
            </span>
            <span>
              <span className="legend-sq" style={{ background: segment.color, opacity: 0.45 }}></span>Planned
            </span>
            <span>
              <span className="legend-sq" style={{ background: "var(--line)" }}></span>Free
            </span>
          </div>
        </div>
